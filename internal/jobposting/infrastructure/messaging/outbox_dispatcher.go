...
// OutboxDispatcher polls the job_posting_outbox table for undispatched
// events and forwards them to an EventPublisher. Mirrors the hiringintent
// dispatcher — different table, different decoder, same locking strategy
...
func NewOutboxDispatcher(pool *pgxpool.Pool, publisher EventPublisher, logger zerolog.Logger, cfg DispatcherConfig) *OutboxDispatcher {
	...
	return &OutboxDispatcher{
		pool:      pool,
		publisher: publisher,
		logger:    logger.With().Str("component", "posting_outbox_dispatcher").Logger(),
...
UPDATE job_posting_outbox SET dispatched_at = NOW() WHERE id = $1
`
...
func decodeEvent(name string, payload []byte) (events.Event, error) {
	switch name {
	case "jobposting.JobPostingCreated":
		var e events.JobPostingCreated
		if err := json.Unmarshal(payload, &e); err != nil {
			return nil, fmt.Errorf("unmarshal %s: %w", name, err)
		}
		return e, nil
	case "jobposting.JobPostingPublished":
		var e events.JobPostingPublished
		if err := json.Unmarshal(payload, &e); err != nil {
			return nil, fmt.Errorf("unmarshal %s: %w", name, err)
		}
		return e, nil
	case "jobposting.JobPostingClosed":
		var e events.JobPostingClosed
		if err := json.Unmarshal(payload, &e); err != nil {
			return nil, fmt.Errorf("unmarshal %s: %w", name, err)
		}
		return e, nil
